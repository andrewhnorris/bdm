...
def violations_per_streetline(output_folder):
	...
	full_violations_joined = full_violations_joined.groupBy('PHYSICALID').agg({'COUNT_2015':'sum','COUNT_2016':'sum','COUNT_2017':'sum','COUNT_2018':'sum','COUNT_2019':'sum'})
	# fill na's with 0
	...
	full_violations_joined = full_violations_joined.withColumn('OLS_COEF', ols_coef(full_violations_joined['sum(2015)'], full_violations_joined['sum(2016)'], full_violations_joined['sum(2017)'], full_violations_joined['sum(2018)'], full_violations_joined['sum(2019)']))
	# order by PHYSICALID
